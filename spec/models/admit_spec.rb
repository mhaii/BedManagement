...
RSpec.describe Admit, type: :model do
  ...
  let(:room) { Room.find_or_create_by({ number: '1412', ward: ward }) }
  let(:patient) { Patient.find_or_create_by({ hn: 1111, first_name: 'first', last_name: 'last'}) }
  ...
  it 'should set status of both rooms when switching between rooms' do
    newRoom  = Room.find_or_create_by({ number: '3412', ward: ward })
    admit = Admit.create({ status: :currentlyAdmit, patient: patient, room: room })
    admit.update room: newRoom

    room.reload

    expect(admit.room).to eq newRoom
    expect(newRoom.status).to eq 'occupied'
    expect(room.status).to eq 'available'
  end
